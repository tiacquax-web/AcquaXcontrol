/**
 * GET /api/v1/apartments — Lista/busca unidades (apartamentos)
 *
 * Autenticação: Bearer Token (API Key)
 * Permissão: apartments:read
 */
...
export async function GET(req: NextRequest): Promise<Response> {
    ...
    const auth = await authenticateApiKey(req, ['apartments:read']);
    if (auth.error) {
        ...
        const [apartments, totalCount] = await Promise.all([
            prisma.apartment.findMany({
                ...
                select: {
                    ...
                    meters: { select: { id: true, register: true, status: true } },
                },
            }),
