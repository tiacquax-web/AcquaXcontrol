...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ entityId: string }> }): Promise<Response> {
    try {
        ...
        const { entity, error: updateError, status: updateStatus } = await updateEntityData(userId, 'role', entityId, body);
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ entityId: string }> }): Promise<Response> {
    try {
        ...
        const { entity, error: deletionError, status: deletionStatus } = await deleteEntity(userId, 'role', entityId);
