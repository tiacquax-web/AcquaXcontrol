...
export async function getAccessibleUserIdsForAction(userId: string, action: UserAction) {
  ...
  const assignmentScopeOr: Array<{ contextType: ContextType; contextId: { in: string[] } }> = [
    ...(companyIds.length > 0 ? [{ contextType: ContextType.company, contextId: { in: companyIds } }] : []),
    ...(complexIds.length > 0 ? [{ contextType: ContextType.complex, contextId: { in: complexIds } }] : []),
    ...(blockIds.length > 0 ? [{ contextType: ContextType.block, contextId: { in: blockIds } }] : []),
    ...(apartmentIds.length > 0 ? [{ contextType: ContextType.apartment, contextId: { in: apartmentIds } }] : []),
  ];
