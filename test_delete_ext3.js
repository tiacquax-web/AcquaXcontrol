...
async function main() {
  ...
  try {
    ...
    console.error('Delete failed:', e.message.substring(0, 300));
  }
  ...
  console.log('Raw in DB:', raw ? `deletedAt=${raw.deletedAt} (${typeof raw.deletedAt})` : 'NOT FOUND - HARD DELETE!');
  
  if (raw) {
    // Try to create another with same socialName
    try {
      ...
      console.log('SUCCESS: Created dup after soft-delete!', dup.id);
      await db.collection('Complexes').deleteOne({ _id: dup.id });
    } catch(e) {
      console.error('FAILED to create dup:', e.code, e.message.substring(0, 300));
    }
  }
